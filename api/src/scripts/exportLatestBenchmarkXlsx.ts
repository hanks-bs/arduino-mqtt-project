...
async function findLatestBenchmarkDir(repoRoot: string) {
  ...
  if (dirs.length === 0) throw new Error('Brak katalogów wyników w api/benchmarks');
...
async function main() {
  ...
  const summary = await fs.readJSON(summaryPath);
  {
    ...
    ws.addRow({ k: 'loadSet', v: (summary.runConfig?.loadSet || []).join(', ') });
    ws.addRow({ k: 'durationSec', v: String(summary.runConfig?.durationSec ?? '') });
    ws.addRow({ k: 'tickMs', v: String(summary.runConfig?.monitorTickMs ?? '') });
    ws.addRow({ k: 'clientsHttp', v: String(summary.runConfig?.clientsHttp ?? '') });
    ws.addRow({ k: 'clientsWs', v: String(summary.runConfig?.clientsWs ?? '') });
  }
  ...
  if (await fs.pathExists(byLoadCsv)) {
    ...
    rows.forEach((r, i) => ws.addRow(i === 0 ? r : r.map(v => (isFinite(Number(v)) ? Number(v) : v))));
  }
  // ByClients sheet
  ...
  if (await fs.pathExists(sessionsCsv)) {
    ...
    subset.forEach((r, i) => ws.addRow(i === 0 ? r : r.map(v => (isFinite(Number(v)) ? Number(v) : v))));
    if (rows.length > MAX) {
      const notes = workbook.getWorksheet('Meta') || workbook.addWorksheet('Meta');
      notes.addRow({ k: 'Sessions truncated', v: `${MAX}/${rows.length} rows saved to XLSX` });
    }
  }
