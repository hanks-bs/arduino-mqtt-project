/*
 ...
 Presets: list, sanity, stable, stable60, quick, safe, robust, clients
*/
...
function printHelp() {
  ...
  ['sanity', '12s, 1 Hz, clients=1/1, pidusage disabled (szybka weryfikacja)'],
    ...
    ['safe', '4s, 0.5–1 Hz, clients=0/0, tick=500ms (bezpieczny minimalny)'],
    ['robust', '60s×2, Hz=0.5,1,2; Load=0,25,50; pair'],
    ['clients', '60s×2, 1 Hz, Load=0,25; clients sets (0,10,25,50); pair'],
  ];
  ...
  switch (preset) {
    ...
    case 'clients':
      await runMeasurements({
        ...
        warmupSec: 4,
        cooldownSec: 4,
        clientsHttpSet: [0, 10, 25, 50],
        clientsWsSet: [0, 10, 25, 50],
        pair: true,
        repeats: 2,
        // @ts-expect-error extended flags supported in CLI
        cpuSampleMs: 1000,
